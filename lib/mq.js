...
module.exports = (app) => {
  ...
  for (const mqItem of mqNames) {
    const mqName = (typeof mqItem === 'string') ? mqItem : mqItem.name;
    const mq = new AliMNS.MQ(mqName, app.mns.account, app.mns.region(app.config.mns.region), app.config.mns.b64Transform);
    assert(!mqMap.has(mqName), `[egg-mns] duplicate Recvp, mqName`);
    mqMap.set(mqName, mq);

    const mqHandlerPath = (typeof mqItem === 'string') ? mqItem : mqItem.handlerName;
    const filepath = path.join(app.config.baseDir, 'app/mns/mq', mqHandlerPath + '.js');
